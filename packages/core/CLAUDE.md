...
- Quickstarts and integration tests that run full-tier registry commands (for example `@agent-os-pkgs/git`) should set both `moduleAccessCwd` and explicit `permissions` on `AgentOs.create()`. Relying on ambient `process.cwd()` can project broken workspace-root symlinks into the VM, and omitting permissions now defaults the native sidecar to deny-all instead of silently allowing access.
